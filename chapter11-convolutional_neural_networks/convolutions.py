...
def convolve(image, K):
    # grab the spacial dimensions of the image and kernel
    (iH, iW) = image.shape[:2]
    (kH, kW) = K.shape[:2]

    # allocate memory for the output image, taking care to "pad"
    # the borders of the input image so the spacial size (i.e. , 
    ...
    for y in np.arange(pad, iH + pad):
        for x in np.arange(pad, iW + pad):
            ...
            # *center* region of the current (x, y)-coordinate's
            # dimensions
            roi = image[y - pad:y + pad + 1, x - pad:x + pad + 1]

            # perform actual convolution by taking the
